...
UMoviePipelinePanoramicPass::UMoviePipelinePanoramicPass() 
	...
	, bStereo(false)
	, EyeSeparation(3.0f)
	, EyeConvergenceDistance(2000000000.f)
...
namespace MoviePipeline
{
	namespace Panoramic
	{
		...
		void GetCameraOrientationForStereo(FVector& OutLocation, FRotator& OutRotation, FRotator& OutLocalRotation, const FPanoPane& InPane, const int32 InStereoIndex, const bool bInPrevPosition, const bool bStereo)
		{
			...
			const TArray<float> YawValues = MoviePipeline::Panoramic::DistributeValuesInInterval(0, 360, InPane.NumHorizontalSteps, /*Inclusive Max*/bStereo);
	...
	for(int32 VerticalStepIndex = 0; VerticalStepIndex < NumVerticalSteps; VerticalStepIndex++)
	{
		...
		for(int32 HorizontalStepIndex = 0; HorizontalStepIndex < NumHorizontalSteps; HorizontalStepIndex++)
		{
			...
			for (int32 EyeLoopIndex = 0; EyeLoopIndex < NumEyeRenders; EyeLoopIndex++)
			{
				...
				MoviePipeline::Panoramic::GetCameraOrientationForStereo(/*Out*/ Pane.PrevCameraLocation, /*Out*/ Pane.PrevCameraRotation, /*Out*/ DummyPrevLocalRot, Pane, StereoIndex, /*bInPrevPos*/ true, bStereo);
