...
EMovieSceneChannelProxyType UTemplateSequenceSection::CacheChannelProxy()
{
	...
	for (FTemplateSectionPropertyScale& PropertyScale : PropertyScales)
	{
		...
		{
			case ETemplateSectionPropertyScaleType::Vector2DProperty:
				ChannelName = FName(*FString::Format(TEXT("{0}[2D Vector]"), { ChannelName.ToString() }));
				break;
			case ETemplateSectionPropertyScaleType::VectorProperty:
				ChannelName = FName(*FString::Format(TEXT("{0}[Vector]"), { ChannelName.ToString() }));
				break;
			case ETemplateSectionPropertyScaleType::Vector4DProperty:
				ChannelName = FName(*FString::Format(TEXT("{0}[4D Vector]"), { ChannelName.ToString() }));
				break;
		...
		// Add the appropriate channel based on property scale type
		if (PropertyScale.PropertyScaleType == ETemplateSectionPropertyScaleType::DoubleProperty)
		{
			Channels.Add(PropertyScale.DoubleChannel, ChannelMetaData, TMovieSceneExternalValue<double>());
		}
		else
		{
		...
		if (PropertyScale.PropertyScaleType == ETemplateSectionPropertyScaleType::DoubleProperty)
		{
			Channels.Add(PropertyScale.DoubleChannel);
		}
		else
		{
...
void UTemplateSequenceSection::ImportEntityImpl(UMovieSceneEntitySystemLinker* EntityLinker, const UE::MovieScene::FEntityImportParams& Params, UE::MovieScene::FImportedEntity* OutImportedEntity)
{
	...
	else if (ensure(PropertyScales.IsValidIndex(Params.EntityID - 1)))
	{
		...
		if (PropertyScale.PropertyScaleType == ETemplateSectionPropertyScaleType::DoubleProperty)
		{
			const bool bHasScale = (PropertyScale.DoubleChannel.GetNumKeys() > 0 || PropertyScale.DoubleChannel.GetDefault().Get(1.0) != 1.0);
			if (bHasScale)
			{
				OutImportedEntity->AddBuilder(
					FEntityBuilder()
						...
						.Add(
							BuiltInComponents->DoubleChannel[0],
							FSourceDoubleChannel(&PropertyScale.DoubleChannel))
						.AddMutualComponents());
			}
		}
		else
		{
			const bool bHasScale = (PropertyScale.FloatChannel.GetNumKeys() > 0 || PropertyScale.FloatChannel.GetDefault().Get(1.f) != 1.f);
			if (bHasScale)
			{
				OutImportedEntity->AddBuilder(
					FEntityBuilder()
