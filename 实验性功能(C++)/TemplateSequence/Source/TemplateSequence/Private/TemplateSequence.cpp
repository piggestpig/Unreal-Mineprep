...
void UTemplateSequence::LocateBoundObjects(const FGuid& ObjectId, UObject* Context, TArray<UObject*, TInlineAllocator<1>>& OutObjects) const
{
	...
	if (UActorComponent* FoundComponent = FindObject<UActorComponent>(Actor, *ComponentName->ToString(), EFindObjectFlags::None))
	{
...
FGuid UTemplateSequence::FindOrAddBinding(UObject* InObject)
{
	...
	ensure(World);
	{
		...
		FGuid FoundBinding;
		for (int32 BindingIndex = 0; !FoundBinding.IsValid() && BindingIndex < ((const UMovieScene*)MovieScene)->GetBindings().Num(); ++BindingIndex)
		{
			for (TWeakObjectPtr<UObject> BoundObject : Player->FindBoundObjects(((const UMovieScene*)MovieScene)->GetBindings()[BindingIndex].GetObjectGuid(), MovieSceneSequenceID::Root))
			{
				if (BoundObject.IsValid() && BoundObject.Get()->GetClass() == InObject->GetClass())
				{
					...
					{
						FoundBinding = ((const UMovieScene*)MovieScene)->GetBindings()[BindingIndex].GetObjectGuid();
						break;
					}
				}
			}
