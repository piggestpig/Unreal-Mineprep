			...
			FUIAction MenuEntryAction = FUIAction(
					FExecuteAction::CreateLambda(
						[NonConstThis, AnimatedProperty]()
						{
							...
							// Set default multiplier value based on property type
							if (NewPropertyScale.PropertyScaleType == ETemplateSectionPropertyScaleType::DoubleProperty ||
								NewPropertyScale.PropertyScaleType == ETemplateSectionPropertyScaleType::Vector2DProperty ||
								NewPropertyScale.PropertyScaleType == ETemplateSectionPropertyScaleType::VectorProperty ||
								NewPropertyScale.PropertyScaleType == ETemplateSectionPropertyScaleType::Vector4DProperty)
							{
								NewPropertyScale.DoubleChannel.SetDefault(1.0);
							}
							else
							{
...
TArray<FTemplateSequenceSection::FScalablePropertyInfo> FTemplateSequenceSection::GetAnimatedProperties() const
{
	...
	for (const FMovieSceneBinding& Binding : SubMovieScene->GetBindings())
	{
		for (const UMovieSceneTrack* BindingTrack : Binding.GetTracks())
		{
			...
			else if (PropertyTrackClass->IsChildOf<UMovieSceneDoubleTrack>())
			{
				SupportedScaleTypes.Add(ETemplateSectionPropertyScaleType::DoubleProperty);
			}
			else if (const UMovieSceneFloatVectorTrack* FloatVectorTrack = Cast<UMovieSceneFloatVectorTrack>(PropertyTrack))
			{
				if (FloatVectorTrack->GetNumChannelsUsed() == 2)
				{
					SupportedScaleTypes.Add(ETemplateSectionPropertyScaleType::Vector2DProperty);
				}
				else if (FloatVectorTrack->GetNumChannelsUsed() == 3)
				{
					SupportedScaleTypes.Add(ETemplateSectionPropertyScaleType::VectorProperty);
				}
				else if (FloatVectorTrack->GetNumChannelsUsed() >= 4)
				{
					SupportedScaleTypes.Add(ETemplateSectionPropertyScaleType::Vector4DProperty);
				}
			}
			else if (const UMovieSceneDoubleVectorTrack* DoubleVectorTrack = Cast<UMovieSceneDoubleVectorTrack>(PropertyTrack))
			{
				if (DoubleVectorTrack->GetNumChannelsUsed() == 2)
				{
					SupportedScaleTypes.Add(ETemplateSectionPropertyScaleType::Vector2DProperty);
				}
				else if (DoubleVectorTrack->GetNumChannelsUsed() == 3)
				{
					SupportedScaleTypes.Add(ETemplateSectionPropertyScaleType::VectorProperty);
				}
				else if (DoubleVectorTrack->GetNumChannelsUsed() >= 4)
				{
					SupportedScaleTypes.Add(ETemplateSectionPropertyScaleType::Vector4DProperty);
				}
			}
