...
- 目前主要有骨骼网格体和自动化NPC两种类型，部分生物制作了顶点动画实例模型，用于大型群体粒子。所有生物都可以更换材质，两脚生物可以添加通用的IK绑定
- 当前版本支持：Steve/Alex玩家、猪、牛、羊、马（驴、骡、僵尸马、骷髅马）、僵尸、尸壳、溺尸、骷髅、凋灵骷髅、流浪者、沼骸、猪灵、猪灵蛮兵、掠夺者、卫道士、铁傀儡、蠹虫、末影螨、蜘蛛（洞穴蜘蛛）。
- 更多内容正在更新中
...
#### 25w25a
- 新增`2D抠像模型`及后期合成相关素材（MC 1.21.6 发布了，最近很有灵感，来做个快乐恶魂的猫meme！）
  - 2D抠像模型是骨骼网格体平面，可以拉伸变形，像生物一样制作动画片段
...
- 新增`遮罩层`，可以让物体仅显示在特定遮罩中（后期剪辑的实用功能get，配合灯光排除能做出更棒的“4D场景”！），后续会改进所有方块和生物的材质
  ...
  - 对于应用遮罩的物体，在材质参数中调整“遮罩层”，逐位过滤遮罩。0为不启用，正数在遮罩内显示（比如12表示接受遮罩层1和2），负数在遮罩外显示（比如-1表示排除遮罩1），如果要同时接受和排除遮罩，用0分割（比如1203表示在遮罩层1、2内+3的外面才显示）
