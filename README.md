...
| 推荐配置 | 参考配置<sup>[1]</sup> |
| :--- | :--- |
| - 支持硬件光线追踪的显卡<sup>[2]</sup> <br>- 32G以上内存和12G以上显存<br>- SSD固态硬盘 | - CPU：i7-13700K<br>- 内存：32G*2 DDR5-6400MHz<br>- 显卡：RTX 4080 (16G)<br>- 硬盘：2TB + 4TB PCIe 4.0 SSD |

<sup> [1] 我们开发插件和制作动画所用的电脑  
...
- 实验性功能是C++代码，必须针对特定平台和特定引擎版本编译。Windows会随每次快照更新，Mac和Linux只会在大版本更新
- 目前物理交互绑定会在Mac和Linux上闪退，安装时将跳过这个素材
- 另外，插件附带了FFmpeg用于视频编码。你也可以手动下载ffmpeg并在安装时指定路径。官方建议下载最新版的master build

> ⚠️  
> MacOS使用过 M4 Max (36G)和 M2 Max (32G)进行测试，目前存在的问题如下：  
> · 尽管能用硬件光追和Megalights，但似乎不支持Alpha遮罩。默认启用VSM虚拟阴影贴图  
> · 远景地形生成器性能差，比预期慢好几倍  
...
- 在导入世界的选项中新增了实验性的`nbt`，`mcstructure`和`schematics`选项。本周重点改进了导入程序，但是仍然有一些问题
  - 导入单个方块会使用特制的单面材质。这解决了花草有重叠面的问题，但是植物摇摆仍会向自身投射阴影
  - 能自动识别动画纹理，使用二级动态材质了
  - 初步支持导入水方块，目前使用不透明材质
  - 自动跳过无法导入的方块，不再中断整个程序
- 改进了统一粒子群体，包括更好的动画系统和基于HLSL的材质。目前最直观的新功能是增加了`装备`参数，可以叠加第二个动画模型
- 修复了对染色玻璃替换半透明材质时错误设置不透明度的bug
- 将在下周修复另一个新发现的bug：爆炸粒子等使用轻量级发射器的系统无法录制缓存


#### 26w25a
- UE5.8发布了！据说这是UE6之前的最后一次大版本更新。我们预计在两周后发布动画视频和Mineprep 0.6-pre1，然后更新至5.8版本
> UE6要等2027年底才出预览版，中间有这么长的空窗期，我们吃什么？  
> 是啊，吃什么？  
> 是时候把Mineprep 1.0提上日程了！
- 继续基于Mineprep Python API开发新功能，并优化插件性能
- 选中多个方块 使用“更换模型（实例化）”，会把每种类型合并为“实例化方块”。如果勾选了“分割为独立的Actors”，则会拆分回去
- 修复了导入火把等方块时模型破损、UV颠倒的问题。不过，门和床这些特殊模型暂时无法正确导入
- 正在编写导入MC结构的程序，有望支持nbt，mcstructure和schematics
  - 当前仅能通过python调用：`mineprep.spawn_structure(filepath='', loc=(0,0,0), rot=(0,0,0), gpu=False)`，下周会加入插件面板
  - 默认情况下，会把每一类方块导入为“实例化方块”
  - 如果使用gpu=True，会把方块的位置和旋转烘焙到两张贴图中，生成一整个“PCG实例化方块”。这由远景地形生成器改造而来，性能极好，但是无碰撞箱且容易消失，渲染开始时会重新生成

#### 26w24a
- 本周在虚幻引擎中编写了放置方块和放置物品的程序，点击后立刻生效，速度和稳定性大幅提升。至此，Mineprep的所有功能都可独立运行，不再需要Blender了！
- Mineprep Python API 转正了！尽管部分函数仍然需要实验性功能才会生效，但是整个框架已不再依赖C++，支持精简版和极简版在内的所有版本
  - 使用方法：直接`import mineprep`，更多信息见Mineprep Wiki
  - 除了原有的模拟点击功能，现在添加了许多专属函数，并特地优化了性能，比如`mineprep.spawn_mob('Steve')`
  - 使用`@mineprep.asynctask`和`yield (秒数)`编写异步延迟函数，例如  
    ```py
    import mineprep
    ...
    def spawn_grid():
	    for i in range(10):
		    for j in range(10):
		      mineprep.spawn_mob('Steve', (i*100,j*100,0))
		      yield

    spawn_grid()
    ```
    这为复杂任务和自动化测试打下基础。
  - 还有很多的语法糖！开发和使用效率空前提高
- “Mineprep自定义快捷键”对象下放到非实验性版本
- 新增`实例化方块`，下周将制作更多功能

#### 26w23a
- 本周大幅改进了画质预设，提升全局光照明并平衡性能。最直观的感受是，日光可以照亮室内了!
  - MC导播台的“画质预设”改为从选项到等级的映射，可以自由组合各种选项
  - 选项分总画质和重载预览画质两类，小参会覆盖大参。等级减少至5级 + 自定义（不重载）。
  - | 画质选项 | -> | 画质等级 |
    | :---: | :---: | :---: |
    | 【全局预设】<sup>#占位符</sup> |  | 0 低 |
    | ★总画质 |  | 1 中 |
    | 引擎画质级别 |  | 2 高 |
    | GI间接光照 |  | 3 极高 |
    | ★细节参数集 |  | 4 影视级 |
    ...
    | → Lumen更新速度 |  |  |
    | → Megalights质量 |  |  |
  - 现在默认启用Lumen命中光照，可通过GI间接光照控制，各等级如下所示：  
    0低：表面缓存 + UE5.8的Lumen辐照场探针  
    1中：反射的命中光照 + UE5.8的Lumen辐照场探针  
    2高：反射的命中光照  
    3极高：命中光照  
    4影视级：命中光照
  > 还记得UE5.5的时候性能很差，现在好起来了，我再微调一下其他参数，帧数几乎不变或略有下降。  
  > 之前过的什么苦日子啊😭有了命中光照，逆光/室内照明/Mojang宣传片风格都好做多了，泪目
  - 默认画质为 ★总画质：4影视级，⇛预览画质：3极高  
    在安装包中勾选优化性能后，默认画质为 ★总画质：3极高，⇛预览画质：2高，→GI间接光照：1中  
    Lumen辐照场探针让帧数在UE5.8直接起飞，不过它会造成大块光影突变，只适合编辑器预览，不适合渲染动画
- 从底层重构了Mineprep Python API，绕过C++直接由Python注入Unreal模块。这一革命性改进有望从实验性功能转正，在兼容旧语法的同时带来更丰富的接口、泛型、热重载。好东西就要来了！
- 修复了导播台重载重力方向在XY轴超过1000时失效报错的bug
    

#### 26w22a
- 改进了方块云和方块体积云，新增MC 1.21.9的新版云朵纹理
  - 方块云改为实例化静态网格体，通过2x2拼接扩大了面积，默认使用旧模型
  - 方块体积云现在能从纹理贴图生成圆角云朵，默认使用新版纹理，调整了密度和颜色使之更加好看，而且性能显著提升
  - 节约储存空间
- 制作了大量实验性功能
  - 调色面板也有关键帧按钮了
  - 能在日志中打印全局快捷键冲突
  - 新增一个捕获后期处理阶段到渲染目标的函数
  - 扩展Mineprep Python API接口
  - 扩展细节面板，自动检测含mover组件的pawn类型角色，将骨骼网格体和材质参数显示在整个Actor的细节面板中。我们预计在UE5.8升级NPC和第三人称运动匹配角色，采用全新的mover移动器（也许还会上UAF动画框架）
- 修复了统一粒子群体在静止或保持直立时方向突变的bug
- 修复了观众人群散布器的朝向
- 修复了LabPBR材质的贴花法线问题
- 修复了几个极简版安装时缺少的素材

#### 26w21a
- 本周对生物、安装包和实验性功能进行大改造
- 开发了全新的放置生物宝宝功能，先尝试放置新版幼年生物，没有则按照老方法缩放成年生物
- 新增`热带鱼`，有两种不同模型，小的算作生物宝宝
- 修改了多层皮肤材质，“纹理颜色”参数只会影响内层，中间层和外层则由“覆层颜色”控制。目前用于村民和热带鱼
- 重新整理了插件结构，在安装包中新增`仅安装核心功能（跳过90%预设素材）`选项，勾选后进行极简安装，最小只需85MB左右！
  - 修复了手动指定FFmpeg路径在渲染时遇到的bug，此时也不再复制FFmpeg到插件目录，节约空间。默认路径指向安装包。
  - 极简版仅包含插件面板中的功能性按钮+Steve/Alex基础模型，剔除了绝大部分预设素材
  - 实验性功能依旧可选，包含快捷键、关键帧等实用功能，非常适合其他非MC工程
  - 整理插件结构可能产生bug，需要更多测试
- 改进了实验性功能
  - 使用新的关键帧按钮图标，默认为空心，打关键帧变实心，操作时间线的程序从蓝图迁移至C++。不过，材质的纹理参数关键帧依旧使用老图标，新图标目前无法在移开播放头时变成同心菱形
  - 添加了捕获视口画面的函数，之后会开发更多功能
  - 改进Mineprep Python API，详见Mineprep Wiki

#### 26w20a
- UE5.8预览版发布了！非实验性功能已适配UE5.8
  - UAF观众人群暂时固定为循环播放动画，避免升级引擎后报错
  - MC天空新增`雾屏幕空间散射`相关参数，能产生朦胧的背景模糊，且支持正交摄像机。需要UE5.8才能生效，目前不能打关键帧
- 继续进行生物模型底层重构
  - 统一了头部骨骼朝向，所有生物都能正确地使用面向目标功能
  - 修复了鹦鹉的缩放
  - 修复了MC_Bent和MC_Segment的模型
- 准备场景的“启用碰撞”选项能同时调整GPU距离场碰撞箱了。修复了nDisplay3D摄像机组的预览窗口带有距离场碰撞箱的bug
- 鸟群粒子默认使用白色鹦鹉皮肤
> 我们已经为UE5.8编译了DLSS插件，做好了升级准备！Mineprep 0.6-pre1预计在一个月内发布，这将是最后一个支持UE5.7的版本  
> 今年的AI终于变聪明了，它能自动搜索引擎安装位置并读取源码，不再凭感觉瞎改程序。我只需要说一声编译DLSS，它就会自主迭代直至编译成功。看来以后能做更多实验性功能了呢！

#### 26w19a
- 大多数生物模型已完成重构，统一面向X轴，接下来需要检查动画片段和NPC是否正常
  - 保留了一个_MC_Retarget_模型面向Y轴，用于动作重定向。更好的方法是在导入动作捕捉片段时直接旋转90°面向X轴，这样就能重定向到任意MC模型了
  - 生成器子面板的生物宝宝也需要重构，适配 MC 26.1 新增的幼年生物模型；同时，热带鱼有两种不同的模型，小的那个也将算作生物宝宝
- 新增`鳕鱼`，`鲑鱼`和`鱼群粒子`。将鱼群粒子放在“高级水体”之下，可以限制它们的活动区域
- 新增MC人模的顶点动画实例模型和`观众人群粒子`。实际上，我们已经有能力为所有生物制作顶点动画和粒子群体了！

#### 26w18a
本周开始对生物模型进行新一轮底层重构：
- 主要任务是把大部分生物从面向y轴改为面向x轴。在此之前，朝向问题已经出现过很多次，像粒子群体、mover移动器、骑乘等功能经常需要打补丁或手动调整。不如从源头上搞定吧
- 同时，修复部分生物的头颈旋转和动画错位问题
- 后续会为头身分离模型补充一体化模型，用于观众人群和粒子群体
- 头部模型暂时不做调整，等未来制作更好的表情功能
> 从Y轴改到X轴，听起来简单，做起来麻烦qwq  
> 现有的动画片段和重定向器不会自动旋转，必须一个个手动调整...  
> 直接回Blender从源头重新导出模型吧

#### 26w17a
本周制作了`高级水体`，功能全方位超越之前的`可交互2D水面`
- 高级水体包含水面材质和水下后期处理材质，放置在普通水面上会直接覆盖它
- 通过调整“波源”参数制造波浪
- 支持在摄像机附近与物体交互，具体范围和精度位于项目设置的“浅水模拟参数”中
- 默认情况下，NPC和玩家在水中移动会自动产生水波；基础生物模型需要通过“附加组件”功能添加碰撞tag；部分粒子系统添加了`与水体碰撞交互`选项，勾选后能产生水波；方块则有单独的交互设置
- 方块添加了`水体交互&漂浮`参数，里面可以设置启用浮力、交互水波、漂浮点位等参数
> 由于水面材质和交互范围不同，“可交互2D水面”依然保留

> 我们的开发重点将转移至具有MC特色的像素水。如果需要高级流体模拟，可以搜索第三方水插件，UE5有很多超棒的插件
- 进一步优化了MC天空的性能
- 【实验性功能】按快捷键 Shift+` 将视口摄像机吸附到前方表面

#### 26w16a
Mineprep 0.6的主题已确定为“水域更新”！
- 新增`水坑贴花`，用于在特定区域生成地面的积水
  - 通过材质参数控制水面反射（金属度/高光度/粗糙度）、噪声、水波、涟漪等效果
  - 增大范围至整个世界，可用于下雨天的地面积水
- MC材质现已支持`孔隙率`参数，实现了LabPBR标准的所有功能！
  - 孔隙率表示吸水程度，目前需要配合水坑贴花使用
  - 这个参数实际上会减弱材质对贴花的金属度、高光度、粗糙度和法线的响应程度
  - 半透明材质不支持贴花，因此孔隙率没有效果。其他材质正在改造中
> 孔隙率（Porosity）并非常见的PBR参数，官方文档也没讲清楚，应该是MC特有的东西。  
> 它竟然和次表面散射打包在一个通道里，还区分金属和非金属，真是绞尽脑汁也想不明白啊()  
> 忽然，我有了一个绝妙的点子，如果用贴花来制作积水效果，就能控制吸水程度了嘛 ✧٩(ˊᗜˋ*)و✧
- 优化了一级动态材质的性能
- 优化了MC天空在编辑器视口拖拽和旋转的性能
...
经过一年多的开发，插件在各个方面都有了重大更新，我们制作了4段动画视频，工作室账号也取得了万粉的成就

你可以前往[Mineprep Wiki](https://github.com/piggestpig/Unreal-Mineprep/wiki)查看新手教程，或者在[讨论区](https://github.com/piggestpig/Unreal-Mineprep/discussions)进行交流。我们已经根据5个社区反馈改进了许多功能，在此表示感谢！

快来看看有哪些新内容吧：

<details>
<summary> 所有核心功能都得到了初步实现 </summary>

- Mineprep 0.5 终于有了
`导入世界`和`更换模型（实例化）`按钮，分别对应MCprep的“OBJ world import”和“Mesh Swap”功能
- 同时，我们正在参考Blender制作Mineprep Python API。安装实验性功能后可以使用Python访问插件面板的每个按钮

</details>

<details>
<summary> 支持LabPBR标准的MC材质 </summary>

- 近几年的MC纹理包大多采用[LabPBR](https://shaderlabs.org/wiki/LabPBR_Material_Standard)标准，Mineprep现已支持除了孔隙率之外的所有特性！
- 新版材质还有更多实用功能，比如半透明、遮罩层、ping-pong动画和多层生物皮肤等

<sup>*孔隙率用于下雨积水时的潮湿效果，这将在Mineprep 0.6（水域更新）中得到支持</sup>
...
- `远景地形生成器`和`动态地形粒子`能在GPU上高效地生成MC地形！它们的性能和开发进度都远远超出预期，是本次更新的最大亮点之一
- Mineprep 0.5的封面图就是用远景地形生成器制作的
- 新的动画视频《？！Bad Apple！？》使用动态地形粒子，可以实时播放
...
- 新增*蠹虫、末影螨、蜘蛛、洞穴蜘蛛、村民、烈焰人、凋灵、疣猪兽、雪傀儡、狼、猫、悦灵、流浪商人、豹猫、恶魂、小恶魂、快乐恶魂、苦力怕、鹦鹉、骆驼、骆驼尸壳、焦骸*
- 使用控制绑定、物理交互、批量播片等多种方式制作动画
- 重新设计了NPC生物，支持随机刷新皮肤和更多运动模式
- 部分生物具有骑乘槽位，并且能够叠高高
...
<details>
<summary> 大统一粒子 </summary>

- 我们设计了MC粒子模板，更新了大量现有的粒子系统，提供统一的基础参数和实用功能
- 使用`全局粒子生成区域`在任意位置生成粒子
- 使用`全局力场`及其衍生的“粒子交互场”、“动画偏移场”控制粒子系统
- 使用`全局区分ID`召唤子粒子，把多个系统联系起来
> 目前采用这一模板的粒子系统有：*下雨粒子、下雪粒子、喷溅药水粒子、灰尘粒子、爆炸粒子、爆装备粒子、爱心粒子、落叶粒子、礼花粒子、粒子自碰撞、统一粒子群体（虫群粒子、蜘蛛群粒子、鸟群粒子、萤火虫粒子、媒体抠像粒子）*
...
<details>
<summary> 更多渲染输出格式 </summary>

> 由于引擎自带的H.264和ProRes编码器只在Windows上可用，我们大幅改进了ffmpeg编码配置，支持MacOS、Linux和高级输出格式

- 视频编码：H.264、H.265、AV1、ProRes
- 图片编码：jpg、png、exr、gif、webp、avif、jxl
- 正确的HDR色彩管理流程，渲染所见即所得的画面
- 更多3D输出格式
- 分层渲染
...
<details>
<summary> 改进用户体验 </summary>

- 修复了许多bug并优化性能；同时UE5.7的材质参数面板不再闪退，这是近期最稳定的一代插件
- 安装包提供更多选项，可以选择精简版，显示所需空间，降低默认画质等
- 插件面板和预设素材有更多本地化翻译
- 正在编写新手教程，详见[Mineprep Wiki](https://github.com/piggestpig/Unreal-Mineprep/wiki)
...
- 这是Mineprep 0.5最后一次计划外的更新，主要根据[社区讨论](https://github.com/piggestpig/Unreal-Mineprep/discussions)中的反馈修复了一些bug并改善用户体验
- 放置方块和放置物品现在有中文翻译了，选中物品旁边还能显示缩略图
- 为“导入世界”添加了`导入到世界原点`选项，修复了放置模型错位的bug
- 导入世界后将在大纲中创建同名文件夹，收纳导入的模型
- 准备场景现在能保护特殊名称的材质，比如背景天空等非LabPBR材质：若材质名以“-”开头，则跳过它；若材质名以“+”开头且是材质实例，则用它创建新的动态材质实例
- 插件面板的材质参数现在能用于实例化网格体
- 远景地形生成器取消勾选在GPU上生成，能自动禁用草的CPU碰撞箱
- 为许多生物添加了骑乘槽位
- 新增`鸟群粒子`和鹦鹉的顶点动画实例模型。实际上我们还做了鱼群粒子，但是现在没有鱼的模型，要等到Mineprep 0.6（暂定为水域更新）才会加入生成器面板
- 渲染SDR视频也可以使用exr作为中间图片格式了，这将充分利用16bit高色深，比8bit png更好。我们将在Mineprep 0.6的第一个快照更新渲染预设
- MacOS默认使用VSM虚拟阴影贴图。光追阴影在测试时始终不支持Alpha遮罩
- 【实验性功能】为MacOS初步适配了3D渲染，不需要nDisplay 3D摄像机组，但是当前不支持分层渲染等功能，也可能存在bug。Windows/Linux用户可以双击“3D”文本来使用这个实验性功能，此时VR前面的勾选框会打上横杠
- 【实验性功能】VR+3D渲染支持全部四种立体格式了，但是目前使用多层exr和引擎内置的视频编码器会有bug
- 【实验性功能】制作了第一代Mineprep Python API，旨在像Blender一样为每个可交互按钮提供python接口，便于编写脚本和自动化测试。更多内容详见Mineprep Wiki
- 使用安装包“新建UE工程文件”，能输入工程文件名称了
- 安装时可以选择`优化性能，降低默认画质`
- 安装插件时会在项目配置中显式启用网格体距离场
- 插件初始化时将打开编辑器偏好设置菜单，加载一些配置
- 修复了NPC生物在待机概率为0时一直不动的bug。这背后修复了与状态树过渡有关的更多概率问题
- 修复了“处理录制的动画”未能生成场景中现有方块被破坏后的粒子轨道的bug。此功能现在会自动运行，已经从插件面板中移除
> 小故事：
> 查看[社区讨论](https://github.com/piggestpig/Unreal-Mineprep/discussions)，获取和本次更新有关的更多信息！

> 在生成器子面板中，方块和物品的名称是从资源包里读出来的，总共有三千多个英文名，很长一段时间都没有翻译。但是，现在已经是2026年了，我忽然意识到，可以开几个agent去找译名嘛！

> 正好上个月的copilot额度还有剩余，我把各种模型都试了一遍，发现最贵的Claude Opus 4.5质量最好。它先从网上找标准译名，再写python脚本，最后自己校对，把翻译率提高到90%以上。虽然有一些问题，但是总算能用了

> 翻译完成后，copilot额度还有剩余，那就再干点什么吧！  
> 如今AI快速发展，命令行强势回归，CLI正在变得比GUI更有效率。要是能像Blender那样，给插件的每个按钮提供Python接口，就能让AI写脚本进行操作了。  
> 于是我又设计了链式调用语法，第一代Mineprep Python API诞生了 \\^o^/

> 做完这个工作后，copilot额度还有剩余，这么好的机会不能浪费，不如...让AI修一下MacOS的3D渲染吧。  
> 一开始我想编译nDisplay插件，但是改到后面发现缺少Nvidia之类的依赖项，看起来没办法解决。然后我又让它改VR渲染插件，把其中的3D渲染拆出来，这个明显简单多了。  
> 当时已是半夜，我提完要求，挂上autopilot自动审批，直接睡了一觉，第二天一看，MacOS的3D渲染在一夜之间建成了！

#### 0.5-RC3
- 0.5-RC3按原计划是最后一次快照更新，包含对粒子系统的重大改进
- 制作了`MC粒子模板`，提供统一的基础参数和实用功能，并用其重置了大量粒子系统
  - 目前有三类基础参数，所有采用MC粒子模板的系统都继承了以下功能：  
    **① 粒子生成**：  
    生成速率/生命周期  
    粒子大小  
    全局区分ID  
    ...
    **③ 子粒子**：  
    ...
    生命周期->每粒子每秒生成  
    数量曲线倍率-单次上限  
  - `全局区分ID`更新为多选框，可以为一个粒子系统分配多个ID，具体内容详见工具提示
  - `初始化批量生成`是一个布尔参数，可以打关键帧，它会在每次从关到开的时候批量生成粒子。如果要多次触发同一粒子系统，这个功能比“触发器轨道”更好用
  - 勾选`批量生成阵列`，会根据阵列数量填充粒子网格，如果批量生成数太少则填不满，生成数太多则会反复填充
  - `子粒子`可以把整个世界中的粒子系统联系起来，制作交互事件。例如，场景中有下雨粒子和爆炸粒子，想让雨滴落地时产生爆炸效果，只需要  
  ① 勾选下雨粒子的“召唤全局子粒子”  
  ② 然后分配一个“子粒子全局ID”（比如1）  
  ③ 接着调整“出生/死亡/碰撞/交互子粒子数”中的碰撞数量（>0）  
  ④ 最后在爆炸粒子中设置对应的“全局区分ID”（包含1即可），它就能在雨滴落地时感受到召唤了！
> 分配ID时一定要注意，自己召唤自己可能使粒子数量指数级增长，几秒内卡死引擎！！
- 新增`全局粒子生成区域`，可以使用全局区分ID在特定地点召唤粒子。默认是方形区域，更改粒子系统后，还有球形、图像、曲线、静态网格体、骨骼网格体这些区域
- 新增`粒子交互场`，当前负数表示杀死粒子，正数表示在粒子进入该场的时候触发子粒子交互事件，≥256会在场中持续触发交互事件
- 新增`礼花粒子`，这是落叶粒子的改版，两者都用上了MC粒子模板
- 更新了MC材质总类，一些粒子系统能对方块染色了
- 新增`动态地形粒子`，由动态设计克隆器和远景地形生成器改造而来
  - 每一帧都能更新地形，可以使用视频纹理作为地形贴图，或者对生成参数打关键帧
  - 兼容动态设计效应器，并提供了“创建链接效应器”按钮
  - 比远景地形生成器多一个“生物群系贴图”，目前是根据图片对方块染色，之后会更新更多功能
- 修复了导播台的DLSS选项修改后无法保存的bug
- 修复了统一粒子群体在摄像机远离又靠近时重新生成的bug
- 修复了全局力场激活休眠对象时导致静态碰撞失效的bug
...
- FFmpeg已更新至最新版master build，添加了prores_ks_vulkan编码器。不过，所有vulkan编码在测试时都有闪烁/花屏问题，暂时无法修复
- 新增12bit 4:2:0、12bit 4:2:2、12bit 4:4:4三种色度抽样选项。CPU H.265编码支持所有9种选项，其他GPU编码可选的条目如下所示：

|   | 8bit 4:2:0 | 10bit 4:2:0 | 10bit 4:2:2 | 10bit 4:4:4 | 12bit 4:2:0 |
| :---: | :---: | :---: | :---: | :---: | :---: |
| Nvidia | ✅ <br> nv12 | ✅ <br> p010le | ℹ️仅50系 <br> p210le | ✅ <br> yuv444p10msble | - |
...
- 新增`分层渲染`，能够把场景中的对象分成多个层，渲染多张背景透明的图片。目前，仅单个层支持ffmpeg视频编码，且光影颜色可能有所不同，未来考虑使用MRG影片渲染图改进分层渲染。
- 使用ProRes时勾选HDR或3D渲染，会主动改用ffmpeg编码，现在能导出透明的HDR视频了！需要注意的是，只有Apple GPU和Vulkan支持ProRes硬件加速，选中Nvidia/Intel/AMD GPU会退回基于CPU的prores_ks编码
- Apple GPU确实不支持av1编码，退回基于CPU的libsvtav1编码
- “渲染带HDR增益图的jpg”即将转正，已经移至顶部菜单栏的“调试与开发”主板块。修复了渲染不在第0帧时的报错问题，改进了HDR色彩管理流程。目前，仅剩下UI界面编辑器等少数功能尚不支持HDR增益图
- 移除了实验性功能`扩展模板序列以支持双精度浮点和向量属性乘数`。此功能在制作动画时从未使用，而且难以编译，在linux上安装会报错，不如直接移除
- 添加了本地化条目，能够翻译变量类别了
- 修复了“远景地形生成器”在mac上因为中文变量名而失效的bug。虽然内部重命名为英文，但是本地化系统能把它翻译成原模原样的中文
- 修复了反斜杠路径名在mac和linux上引发的错误
- 修复了mac和linux不能使用tkinter打开文件的问题，转而使用系统自带的文件浏览器
- 修复了mac和linux上可执行文件缺少权限的问题。使用安装包或重新初始化插件时，都会为ffmpeg和ultrahdr_app注入rwx权限
- 修复了mac和linux缺少内置的H.264与ProRes编码器的问题，改用ffmpeg编码。可惜的是，mac还缺少nDisplay插件，目前没有替代方案，因此不能进行3D立体渲染
> 虚幻引擎震撼首发：[Apple ProRes Movie ...... 只在Windows构建中可用！](https://dev.epicgames.com/documentation/zh-cn/unreal-engine/movie-render-graph-nodes-in-unreal-engine#outputfiletype%E8%8A%82%E7%82%B9)  
> 不是哥们 这什么NTR行为啊喂🤣
- 更多mac和linux的兼容性问题见[系统要求](#系统要求)部分
...
- 新的动画作品《捡到一只快乐小恶魂2》现已发布！第三集正在制作中，小恶魂的故事就要完结啦
  - 在制作动画的过程中，我们检验了现有功能并修复了许多bug。新功能直接在工程文件中开发，注重参数化和拓展性
  - DLSS光线重构首次投入使用，极大提升了水面反射和半透明阴影的质量
  - 这是第一个使用正确的HDR色彩管理流程渲染的视频，配合LabPBR材质，画面相较于第一集有了质的飞跃
  - UE5.7本身就是一个很好的版本。一方面，材质参数面板十分稳定，大幅减少了闪退次数；另一方面，缓动曲线预设从动态设计序列下放到普通关卡序列，用起来非常方便
- 新增`鹦鹉`，`骆驼`，`骆驼尸壳`，`焦骸`及其NPC生物。鹦鹉NPC能根据随机种子刷新皮肤，骆驼会自动扇耳朵
- 修改和添加了几个摄像机晃动预设
- 修改和添加了一些粒子系统，还没有放入生成器子面板中
- 添加了一些生物的动画片段
- 添加了一些方块模型
- 修复了飞行NPC在原地不动的bug。之后会进一步修改状态树
- 修复了插件面板在初始化时未能注入本地化材质参数名的bug
- 拓展了本地化系统，原先零零散散分布在蓝图中的“简单中英文选择器”，也能被收录在[变量显示名_VariableDisplayNames.csv](./Mineprep/插件贴图/变量显示名_VariableDisplayNames.csv)里面了。多行文本有一些问题，因为换行符可能是\r\n而不是\n

#### 26w07a
- 要过年啦！！在这辞旧迎新之际，我们修复了近期最重大的bug，调整了一部分动画相关功能
- 引擎自带的Anim Mixer插件已被禁用。它会强制使用自定义动画模式，使动画蓝图失效。更多信息参见 [https://forums.unrealengine.com/t/ue5-7-preview-sequencer-forces-custom-mode-overrides-animation-blueprint/2663672](https://forums.unrealengine.com/t/ue5-7-preview-sequencer-forces-custom-mode-overrides-animation-blueprint/2663672)
- 没有Anim Mixer插件，暂时无法在时间线上使用动画序列控制新版观众人群（UAF动画框架）。“切换动作”事件仍然有效
- 为媒体抠像播放器制作了动画蓝图，用于混合默认动画和时间线上的动画序列
- 基础生物模型和NPC生物新增`面向玩家/目标`、`面向角度/速度/始末距离`参数，可以自动转头面向最近的目标。部分旧的生物模型由于骨架不同，旋转方向可能不对
- 骑乘者现在会自动对齐旋转
- 村民NPC能根据随机种子自动刷新皮肤了
- 远景地形生成器新增`地形边界衰减`参数，用于降低边界高度
- 导播台新增`优化/还原`按钮，点击可以优化简单MC场景的性能
> Mineprep 0.5已经不知不觉做了快一年了，下周一定要发布新的动画，然后速速结束这个版本呀！！或许我选择“荒野更新”这个大版本主题就是注定要拖更的 (´。＿。｀)  
> 荒野更新（x）  洞穴与山崖part3（√）
