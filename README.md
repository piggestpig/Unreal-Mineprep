...
- 渲染输出模块新增AV1视频编码、线性颜色选项和`HDR静帧_2K_avif`影片图渲染预设。通过一级调色还原lut极大改进了HDR渲染的色彩。
...
> Windows建议使用chrome浏览器查看avif，记得要开启HDR显示；苹果原生支持avif（但是不支持av1视频），亮度好像会暗一点
- HDR_2K_exr渲染预设更改为`【线性】2K_exr`，双目立体全景渲染从exr更改为`【实验性】VR_3D-FTB_8K_jpg`
  - 带着Rec2100 PQ还没一级调色的exr太少见了，通常就是线性和ACES；另外单独渲染序列帧是没有调色的，所以双目立体全景渲染退回jpg了，双击点进去也能改回exr。
