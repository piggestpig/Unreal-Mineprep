...
- **插件面板**上方有选择语言的按钮，在启动时会根据UE的偏好设置选择语言。本地化内容保存在[语言本地化_language_localization.csv](./Mineprep/插件贴图/语言本地化_language_localization.csv) 和 [变量显示名_VariableDisplayNames.csv](./实验性功能(C++)/Mineprep/Content/变量显示名_VariableDisplayNames.csv)中。
...
- 【实验性功能】增强了本地化系统，能够修改引擎内置的文本了！在[变量显示名_VariableDisplayNames.csv](./Mineprep/插件贴图/变量显示名_VariableDisplayNames.csv)的第一列添加了Namespace，用于区分不同板块。在插件面板中勾选“朗读鼠标下文本”，会在日志中打印读取到的本地化信息，把Namespace、Key和Source String复制到csv文件的前三列，然后就能自定义翻译文本了
...
- 修复了DLSS模式失效的bug，现在默认启用光线重构，大幅提升了半透明阴影清晰度。修复了半透明阴影在远处消失的问题
- 修改了画质预设，移除了反射为极高和4*选项，改为
  - 3- 极高（快速Lumen）
  - 3  极高
  - 3+ 极高（特调画质）
  - 4- 影视级
