...
<img src="/Readme素材/插件展示.jpg" alt="示例图片">
...
## 开源协议
- 本插件基于GPL-3.0协议开源，这也是继承自MCprep原作者所使用的协议
- 在这里感谢所有引用到的软件和插件，包括Unreal Engine，Minecraft，Blender，MCprep，TApython，DLSS等，使用相关内容时也可以去看看它们的要求———例如MCprep要求你拥有正版的Minecraft，虚幻引擎官方素材只能在虚幻引擎中使用。
- 免责声明：虚幻引擎是重量级软件，插件里也有重量级的流体模拟和渲染预设......~~如果用着用着C盘爆了~~ 别装C盘！ ~~闪退或者蓝屏~~ 检查内存和CPU稳定性！使用不当不是我的问题 X﹏X

## 版本更新

#### 0.4-pre1
![image](Readme素材/0.4封面图.jpg)
- 新的里程碑(*・ω・)ﾉ 我们在这一周修复了大量bug并带来几个新功能，稍后将上传打包的exe示例文件，包含0.3和0.4-pre1的更新内容，作为正式的Release发布
...
- **实验性功能**  
  ...
  `效果`：将改写后的新插件解压并移动至工程文件根目录的Plugins文件夹下，重新启动时会优先从这里加载插件。全景渲染中的Stereo、Eye Separation和Eye Convergence Distance选项已经解锁，可以输出双目立体全景渲染，其他渲染预设不受影响。暂未加入插件面板的自定义渲染配置。  
  `可选项`：是/否
- 新的渲染预设`【实验性】VR_3D-FTB_8K_ACES_exr`，需要开启实验性功能  
  - 使用8640*8640分辨率，输出上下并列的Full Top-Bottom全景序列帧，采用ACEScg色彩变换和DWAA压缩的exr编码。
  - 既然要做这么高规格的渲染，那各项参数都拉满吧，一帧约30MB，需要约40G的显存+共享内存......讲个鬼故事，png甚至会比exr大得多，这真不是普通电脑能带的动的qwq
- 继续修复渲染预设和人物碰撞bug
- 现在可以场景中放置多个“软体史蒂夫”了
...
- 插件面板弹出框现已支持英语，Github Readme也翻译了一部分英语 —— 是的，你点最顶上那个English按钮不会跳到404页面了，最古老的bug终于被修复了（doge）
