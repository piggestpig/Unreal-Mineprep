...
除了[虚幻引擎官方文档](https://dev.epicgames.com/documentation/zh-cn/unreal-engine/hardware-and-software-specifications-for-unreal-engine)中列出的要求之外，渲染动画还需要更高的配置和更多的显存：

### 最低配置
- 支持硬件光线追踪的显卡  
(NVIDIA 20系，AMD 6000系，Intel Arc 或 Apple M3 及以上)
- 32G以上内存和10G以上显存
- SSD固态硬盘

### 推荐配置
以下是我们开发插件与制作动画所用的电脑，可供参考
- CPU：i7-13700K
- 内存：32G*2 DDR5-6400MHz
- 显卡：RTX 4080 (16G)
- 硬盘：2TB + 4TB PCIe 4.0 SSD
- 系统：Windows 11
...
> · 可以使用硬件光线追踪和Megalights，但似乎不支持Alpha蒙版，亮度也较暗  
> · 远景地形生成器性能差  
> · 可交互3D水面非常不稳定  
> · 缺少nDisplay，不支持3D立体渲染  
> · 缺少tkinter，无法显示Ctrl+Alt+?的快捷键提示等面板  
> · 快捷键的Ctrl似乎会变成Cmd，远程连接时一个快捷键也没按出来

> ⚠️  
> Linux使用E5+64G+3090的服务器测试，除了缺少tkinter外，暂时没发现大问题  
...
· 充足的显存是流畅运行的基础，渲染动画会额外占用更多显存。DX12（Windows默认的图形API）在爆显存后能使用共享内存继续渲染，此时速度会慢10倍左右。Vulkan爆显存则会直接崩溃。
...
- 第一个实验性功能还提供了自定义快捷键，可以按insert尝试翻译鼠标下的文本。如果在变量显示名的csv文件中找到匹配条目，就替换文本
...
- 新增12bit 4:2:0、12bit 4:2:2、12bit 4:4:4三种色度抽样选项。CPU H.265编码支持所有9种选项，其他GPU编码所选择的条目如下所示：
...
- 移除了实验性功能`扩展模板序列以支持双精度浮点和向量属性乘数`。此功能在制作动画时从未使用，而且难以编译，在linux上单独使用会报错，不如直接移除
- 添加了本地化条目，能够翻译变量类别了
...
> 虚幻引擎震撼首发：[Apple ProRes Movie ... 只在Windows构建中可用！](https://dev.epicgames.com/documentation/zh-cn/unreal-engine/movie-render-graph-nodes-in-unreal-engine#outputfiletype%E8%8A%82%E7%82%B9)不是哥们 这什么NTR行为啊喂🤣
