...
> ⚠️  
> MacOS使用M4 Max + 36G内存进行测试，目前未能修复的问题如下：  
> · 尽管能用硬件光追和Megalights，但似乎不支持Alpha蒙版，亮度也较暗  
> · 远景地形生成器性能差，比预期慢10倍以上  
> · 可交互3D水池非常不稳定，产生剧烈的自碰撞  
> · 缺少nDisplay，不支持3D立体渲染  
> · 缺少tkinter，无法显示快捷键提示等面板  
> · 快捷键的Ctrl似乎会变成Cmd，远程连接时一个快捷键也没按出来

> ⚠️  
> Linux使用E5+64G+3090的服务器测试  
> · 除了缺少tkinter外，暂时没发现大问题  
> · E5实在是太太太慢了，别看有18个核心，编译着色器竟然花了半小时，进编辑器只有四五十帧qwq （我第一次直观感受到单核性能的重要性）  
> · 真的有人用linux做动画吗？

· 充足的显存是流畅运行的基础，渲染动画会额外占用更多显存。Windows默认使用DX12作为图形API，爆显存后能使用共享内存继续渲染，但速度会慢10倍左右。Vulkan爆显存则会直接崩溃。
...
- 制作了`MC粒子模板`，提供统一的基础参数和实用功能，并用其重置了大量粒子系统
  - 目前有三类基础参数，所有采用MC粒子模板的系统都继承了以下功能：  
    **① 粒子生成**：生成速率/生命周期，粒子大小，全局区分ID，生成区域大小，生成位置偏移，初始化批量生成，批量生成数/间隔时间，批量生成阵列，阵列XYZ数量  
    **② 力场**：重载重力XYZ-权重W，全局力场权重，全局力场采样类型  
    **③ 子粒子**：召唤全局子粒子，子粒子全局ID，出生/死亡/碰撞/交互子粒子数，生命周期->每粒子每秒生成，数量曲线倍率-单次上限  
  ...
  > 自己召唤自己很可能使粒子数量指数级增长，几秒内卡死引擎，分配ID时一定要注意！！
