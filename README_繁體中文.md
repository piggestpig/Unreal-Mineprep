...
繁體中文 | [**English**](./README_English.md) | [**中文**](./README.md)
...
在v1.0正式發佈前，插件主要供內部使用，可能存在兼容性問題和潛在的 ~~bug~~ 特性。之後會逐步修復並寫一些[ 使用教程（點擊查看）](https://github.com/piggestpig/Unreal-Mineprep/wiki)

[Mineprep Lite](https://github.com/piggestpig/Mineprep-Lite)是此插件的精簡版。它的體積更小，下載更快，以GPLv3協議開源，保留了95%的核心功能，壓縮包僅250MB左右

## 安裝方法

### ① 使用安裝包（推薦，適用於0.4+版本）
1、前提條件：安裝 [虛幻引擎](https://www.unrealengine.com/zh-CN/download)、[Blender](https://www.blender.org/download/) 和 [MCprep](https://theduckcow.com/dev/blender/mcprep/) 插件
> MacOS用戶還需安裝 [Xcode](https://developer.apple.com/xcode/) 才能啓動虛幻引擎

2、下載Mineprep，建議保存在純英文路徑下
> 你可以前往 [Releases](https://github.com/piggestpig/Unreal-Mineprep/releases) -> Assets -> Source code
(zip) 下載較爲穩定的大版本  
> 或者點擊上面的綠色 Code 按鈕 -> Download ZIP，下載最新快照版本。

3、解壓插件安裝包，使用Blender打開裏面的“Mineprep_installer.blend”文件

4、點擊▶️運行腳本，會彈出菜單和安裝引導，目前支持“創建新的工程文件”或“安裝至現有工程”
> 你可以選擇實驗性功能、修改插件設置，安裝所需空間在500MB~2GB之間

### ② 手動安裝
需要複製Mineprep內容文件夾+修改項目設置，稍後會更新詳細步驟

<details>
<summary> 查看安裝時修改的文件 </summary>
...
<details>
<summary> [推薦] 安裝DLSS </summary>

非常建議N卡用戶安裝DLSS，能提升性能、提高畫質、節約顯存

1、前往[https://developer.nvidia.com/rtx/dlss#getstarted](https://developer.nvidia.com/rtx/dlss#getstarted)，在“DLSS 4 Plugin for Unreal Engine”板塊下載對應的插件版本

2、解壓後，複製Plugins文件夾裏的子文件夾 到虛幻引擎插件目錄的Marketplace文件夾中，如下所示:
...
Engine
  ...
  └── ...
```
沒有Marketplace文件夾的話自己新建一個

3、打開工程文件，在左上角-編輯-插件 裏搜索DLSS，啓用`NVIDIA
DLSS
Super Resolution/Ray Reconstruction/DLAA`和`Movie Render Queue DLSS/DLAA Support`，重啓引擎

安裝成功後，你可以在MC導播臺中調整DLSS擋位。

> 渲染動畫時只能使用DLSS和光線重構，用不了幀生成，所以啓用上面兩個插件就夠了
...
## 系統要求
Mineprep 0.5 使用 Windows + UE5.7 開發，建議在此環境下使用。其他平臺或引擎版本將會有一些功能不可用。

除了[虛幻引擎官方文檔](https://dev.epicgames.com/documentation/zh-cn/unreal-engine/hardware-and-software-specifications-for-unreal-engine)中的最低要求之外，渲染動畫還需要更高的配置和更多的顯存：

| 推薦配置 | 參考配置<sup>[1]</sup> |
| :--- | :--- |
| - 支持硬件光線追蹤的顯卡<sup>[2]</sup> <br>- 32G以上內存和12G以上顯存<br>- SSD固態硬盤 | - CPU：i7-13700K<br>- 內存：32G*2 DDR5-6400MHz<br>- 顯卡：RTX 4080 (16G)<br>- 硬盤：2TB + 4TB PCIe 4.0 SSD |

<sup> [1] 我們開發插件和製作動畫所用的電腦  
[2] NVIDIA 20系，AMD 6000系，Intel Arc 或 Apple M3 及以上 </sup>


### 平臺兼容性

| *Mineprep 0.5* | 安裝包(Blender) | UE5.7 | 高版本引擎(5.8+) | 低版本引擎(5.6-) |
| :---: | :---: | :---: | :---: | :---: |
| Windows | ✅ | ✅ | ⚠️ | ❌ |
|   Mac   | ✅ | ✅* | ⚠️ | ❌ |
|  Linux  | ✅ | ✅* | ⚠️ | ❌ |
...
✅：兼容，已經過測試，能夠正常使用  
ℹ️：尚未測試，理論上應該可用  
⚠️：**實驗性功能不可用**，非實驗性功能應該可用  
❌：不兼容，無法使用

> [!WARNING]  
> 請勿在不兼容的環境（比如高版本引擎）勾選實驗性功能，否則工程文件打都打不開，一上來就報錯！！
...
- 插件的非實驗性功能是UE5藍圖資產，高版本引擎可以向下兼容舊資產，但低版本引擎無法打開新資產。如果你不得不使用低版本引擎，請下載Mineprep的早期版本
- 實驗性功能是C++代碼，必須針對特定平臺和特定引擎版本編譯。Windows會隨每次快照更新，Mac和Linux只會在大版本更新
- 目前物理交互綁定會在Mac和Linux上閃退，安裝時將跳過這個素材
- 另外，插件附帶了FFmpeg用於視頻編碼。你也可以手動下載ffmpeg並在安裝時指定路徑。官方建議下載最新版的master build

> ⚠️  
> MacOS使用過 M4 Max (36G)和 M2 Max (32G)進行測試，目前存在的問題如下：  
> · 儘管能用硬件光追和Megalights，但似乎不支持Alpha遮罩。默認啓用VSM虛擬陰影貼圖  
> · 遠景地形生成器性能差，比預期慢好幾倍  
> · 可交互3D水池非常不穩定，產生劇烈的自碰撞  
> · 沒有DLSS等超分辨率插件，默認使用引擎自帶的TSR，拖影嚴重且性能差，空場景佔用6ms比光追還慢  
> · 缺少nDisplay，不支持3D立體渲染，安裝實驗性功能才後有初步的3D渲染功能  
> · 缺少tkinter，無法顯示快捷鍵提示等面板  
> · Mac的文件夾可以隨意擺放文件位置，渲染後好像要向下滾動才能在底部看到視頻  
> · 生成器子面板的排序和Windows不一樣，好像是python sort的問題，反正中文名都沒啥順序，就當特性好了  
> · 快捷鍵的Ctrl似乎會變成Cmd，遠程連接時一個快捷鍵也沒按出來，本地使用應該沒問題

> ⚠️  
> Linux使用E5+64G+3090的服務器測試，只簡短地測過一次  
> · 除了缺少tkinter外，暫時沒發現大問題  
> · E5實在是太太太慢了，別看有18個核心，編譯着色器竟然花了半小時，進編輯器只有四五十幀qwq （我第一次直觀感受到單核性能的重要性）  
> · 真的有人用linux做動畫嗎？

> ⚠️  
> 使用NvRTX等非官方引擎版本，實驗性功能不可用，其他功能應該可用  
> 非官方版本一般需要自己編譯，假如你已經配好了環境，其實可以安裝實驗性功能，在打開工程文件時根據提示重新編譯  
> NvRTX 5.7僅支持Blendable Gbuffer，安裝插件時請勾選“使用兼容性更好的有限Blendable GBuffer材質”

· 充足的顯存是流暢運行的基礎，渲染動畫會額外佔用更多顯存。Windows(DX12)和MacOS在爆顯存後能使用內存/硬盤繼續渲染，但速度會慢10倍左右。Vulkan爆顯存則會直接崩潰。

· 使用推薦配置中的RTX4080主機制作動畫，編輯器中的幀率通常在60~120fps之間

· 我們常用的渲染預設是 `2K_120.mp4` 和 `2K_120_HDR.mp4`，使用 `4 影視級（特調畫質）`，開啓DLAA和DLSS光線重構，輸出幀率在10~30fps左右
...
- 當前版本支持：Steve/Alex玩家、豬、牛、羊、馬（驢、騾、殭屍馬、骷髏馬）、喪屍(殭屍)、屍殼、溺屍(沉屍)、骷髏、凋零骷髏、流浪者(流髑)、沼骸、豬人(豬布林)、殘暴豬人(豬布林蠻兵)、掠奪者、衛道士、鐵魔像(鐵巨人)、蠹魚、終界蟎、蜘蛛（洞穴蜘蛛）、村民、烈焰使者、凋零、疣豬獸、雪人、狼、貓、悅靈、流浪商人、豹貓(山貓)、地獄幽靈、小幽靈、快樂幽靈、苦力怕、鸚鵡、駱駝、駱駝屍殼、旱骨(枯骸)
- 更多內容正在更新中
...
- 只有安裝了第一個實驗性功能 `(Mineprep C++ 拓展模塊)` 才能翻譯粒子參數  
- 只有安裝了第二個實驗性功能 `(爲材質參數面板添加關鍵幀按鈕和本地化翻譯)` 才能翻譯材質參數
- 第一個實驗性功能還提供了自定義快捷鍵，可以按`insert`嘗試翻譯鼠標下的文本。如果在變量顯示名的csv文件中找到匹配條目，就替換文本

更多內容正在更新中

## 開源協議
- 由於完整版Mineprep包含了虛幻引擎官方示例和源代碼，它們和開源許可證不兼容，這裏就先不放License文件了。[Mineprep Lite](https://github.com/piggestpig/Mineprep-Lite) 是此插件的精簡版，它以GPLv3協議開源
...
經過一年多的開發，插件在各個方面都有了重大更新，我們製作了4段動畫視頻，工作室賬號也取得了萬粉的成就

你可以前往[Mineprep Wiki](https://github.com/piggestpig/Unreal-Mineprep/wiki)查看新手教程，或者在[討論區](https://github.com/piggestpig/Unreal-Mineprep/discussions)進行交流。我們已經根據5個社區反饋改進了許多功能，在此表示感謝！

快來看看有哪些新內容吧：

<details>
<summary> 所有核心功能都得到了初步實現 </summary>

- Mineprep 0.5 終於有了
`導入世界`和`更換模型（實例化）`按鈕，分別對應MCprep的“OBJ world import”和“Mesh Swap”功能
- 同時，我們正在參考Blender製作Mineprep Python API。安裝實驗性功能後可以使用Python訪問插件面板的每個按鈕

</details>

<details>
<summary> 支持LabPBR標準的MC材質 </summary>

- 近幾年的MC紋理包大多采用[LabPBR](https://shaderlabs.org/wiki/LabPBR_Material_Standard)標準，Mineprep現已支持除了孔隙率之外的所有特性！
- 新版材質還有更多實用功能，比如半透明、遮罩層、ping-pong動畫和多層生物皮膚等

<sup>*孔隙率用於下雨積水時的潮溼效果，這將在Mineprep 0.6（水域更新）中得到支持</sup>

</details>

<details>
<summary> 程序化生成MC地形 </summary>

- `遠景地形生成器`和`動態地形粒子`能在GPU上高效地生成MC地形！它們的性能和開發進度都遠遠超出預期，是本次更新的最大亮點之一
- Mineprep 0.5的封面图就是用遠景地形生成器製作的
- 新的動畫視頻《？！Bad Apple！？》使用動態地形粒子，可以即時播放
...
- 新增*蠹魚、終界蟎、蜘蛛、洞穴蜘蛛、村民、烈焰使者、凋零、疣豬獸、雪人、狼、貓、悅靈、流浪商人、豹貓(山貓)、地獄幽靈、小幽靈、快樂幽靈、苦力怕、鸚鵡、駱駝、駱駝屍殼、旱骨(枯骸)*
- 使用控制綁定、物理交互、批量播片等多種方式製作動畫
- 重新設計了NPC生物，支持隨機刷新皮膚和更多運動模式
- 部分生物具有騎乘槽位，並且能夠疊高高
...
<details>
<summary> 大統一粒子 </summary>

- 我們設計了MC粒子模板，更新了大量現有的粒子系統，提供統一的基礎參數和實用功能
- 使用`全局粒子生成區域`在任意位置生成粒子
- 使用`全局力場`及其衍生的“粒子交互場”、“動畫偏移場”控制粒子系統
- 使用`全局區分ID`召喚子粒子，把多個系統聯繫起來
> 目前採用這一模板的粒子系統有：*下雨粒子、下雪粒子、噴濺藥水粒子、灰塵粒子、爆炸粒子、爆裝備粒子、愛心粒子、落葉粒子、禮花粒子、粒子自碰撞、統一粒子羣體（蟲羣粒子、蜘蛛羣粒子、鳥羣粒子、螢火蟲粒子、媒體摳像粒子）*
...
<details>
<summary> 更多渲染輸出格式 </summary>

> 由於引擎自帶的H.264和ProRes編碼器只在Windows上可用，我們大幅改進了ffmpeg編碼配置，支持MacOS、Linux和高級輸出格式

- 視頻編碼：H.264、H.265、AV1、ProRes
- 圖片編碼：jpg、png、exr、gif、webp、avif、jxl
- 正確的HDR色彩管理流程，渲染所見即所得的畫面
- 更多3D輸出格式
- 分層渲染
...
<details>
<summary> 改進用戶體驗 </summary>

- 修復了許多bug並優化性能；同時UE5.7的材質參數面板不再閃退，這是近期最穩定的一代插件
- 安裝包提供更多選項，可以選擇精簡版，顯示所需空間，降低默認畫質等
- 插件面板和預設素材有更多本地化翻譯
- 正在編寫新手教程，詳見[Mineprep Wiki](https://github.com/piggestpig/Unreal-Mineprep/wiki)
...
- 這是Mineprep 0.5最後一次計劃外的更新，主要根據[社區討論](https://github.com/piggestpig/Unreal-Mineprep/discussions)中的反饋修復了一些bug並改善用戶體驗
- 放置方塊和放置物品現在有中文翻譯了，選中物品旁邊還能顯示縮略圖
- 爲“導入世界”添加了`導入到世界原點`選項，修復了放置模型錯位的bug
- 導入世界後將在大綱中創建同名文件夾，收納導入的模型
- 準備場景現在能保護特殊名稱的材質，比如背景天空等非LabPBR材質：若材質名以“-”開頭，則跳過它；若材質名以“+”開頭且是材質實例，則用它創建新的動態材質實例
- 插件面板的材質參數現在能用於實例化網格體
- 遠景地形生成器取消勾選在GPU上生成，能自動禁用草的CPU碰撞箱
- 爲許多生物添加了騎乘槽位
- 新增`鳥羣粒子`和鸚鵡的頂點動畫實例模型。實際上我們還做了魚羣粒子，但是現在沒有魚的模型，要等到Mineprep 0.6（暫定爲水域更新）纔會加入生成器面板
- 渲染SDR視頻也可以使用exr作爲中間圖片格式了，這將充分利用16bit高色深，比8bit png更好。我們將在Mineprep 0.6的第一個快照更新渲染預設
- MacOS默認使用VSM虛擬陰影貼圖。光追陰影在測試時始終不支持Alpha遮罩
- 【實驗性功能】爲MacOS初步適配了3D渲染，不需要nDisplay 3D攝像機組，但是當前不支持分層渲染等功能，也可能存在bug。Windows/Linux用戶可以雙擊“3D”文本來使用這個實驗性功能，此時VR前面的勾選框會打上橫槓
- 【實驗性功能】VR+3D渲染支持全部四種立體格式了，但是目前使用多層exr和引擎內置的視頻編碼器會有bug
- 【實驗性功能】製作了第一代Mineprep Python API，旨在像Blender一樣爲每個可交互按鈕提供python接口，便於編寫腳本和自動化測試。更多內容詳見Mineprep Wiki
- 使用安裝包“新建UE工程文件”，能輸入工程文件名稱了
- 安裝時可以選擇`優化性能，降低默認畫質`
- 安裝插件時會在項目配置中顯式啓用網格體距離場
- 插件初始化時將打開編輯器偏好設置菜單，加載一些配置
- 修復了NPC生物在待機概率爲0時一直不動的bug。這背後修復了與狀態樹過渡有關的更多概率問題
- 修復了“處理錄製的動畫”未能生成場景中現有方塊被破壞後的粒子軌道的bug。此功能現在會自動運行，已經從插件面板中移除
> 小故事：
> 查看[社區討論](https://github.com/piggestpig/Unreal-Mineprep/discussions)，獲取和本次更新有關的更多信息！

> 在生成器子面板中，方塊和物品的名稱是從資源包裏讀出來的，總共有三千多個英文名，很長一段時間都沒有翻譯。但是，現在已經是2026年了，我忽然意識到，可以開幾個agent去找譯名嘛！

> 正好上個月的copilot額度還有剩餘，我把各種模型都試了一遍，發現最貴的Claude Opus 4.5質量最好。它先從網上找標準譯名，再寫python腳本，最後自己校對，把翻譯率提高到90%以上。雖然有一些問題，但是總算能用了

> 翻譯完成後，copilot額度還有剩餘，那就再幹點什麼吧！  
> 如今AI快速發展，命令行強勢迴歸，CLI正在變得比GUI更有效率。要是能像Blender那樣，給插件的每個按鈕提供Python接口，就能讓AI寫腳本進行操作了。  
> 於是我又設計了鏈式調用語法，第一代Mineprep Python API誕生了 \\^o^/

> 做完這個工作後，copilot額度還有剩餘，這麼好的機會不能浪費，不如...讓AI修一下MacOS的3D渲染吧。  
> 一開始我想編譯nDisplay插件，但是改到後面發現缺少Nvidia之類的依賴項，看起來沒辦法解決。然後我又讓它改VR渲染插件，把其中的3D渲染拆出來，這個明顯簡單多了。  
> 當時已是半夜，我提完要求，掛上autopilot自動審批，直接睡了一覺，第二天一看，MacOS的3D渲染在一夜之間建成了！

#### 0.5-RC3
- 0.5-RC3按原計劃是最後一次快照更新，包含對粒子系統的重大改進
- 製作了`MC粒子模板`，提供統一的基礎參數和實用功能，並用其重置了大量粒子系統
  - 目前有三類基礎參數，所有采用MC粒子模板的系統都繼承了以下功能：  
    **① 粒子生成**：  
    生成速率/生命週期  
    粒子大小  
    全局區分ID  
    ...
    **③ 子粒子**：  
    ...
    生命週期->每粒子每秒生成  
    數量曲線倍率-單次上限  
  - `全局區分ID`更新爲多選框，可以爲一個粒子系統分配多個ID，具體內容詳見工具提示
  - `初始化批量生成`是一個布爾參數，可以打關鍵幀，它會在每次從關到開的時候批量生成粒子。如果要多次觸發同一粒子系統，這個功能比“觸發器軌道”更好用
  - 勾選`批量生成陣列`，會根據陣列數量填充粒子網格，如果批量生成數太少則填不滿，生成數太多則會反覆填充
  - `子粒子`可以把整個世界中的粒子系統聯繫起來，製作交互事件。例如，場景中有下雨粒子和爆炸粒子，想讓雨滴落地時產生爆炸效果，只需要  
  ① 勾選下雨粒子的“召喚全局子粒子”  
  ② 然後分配一個“子粒子全局ID”（比如1）  
  ③ 接着調整“出生/死亡/碰撞/交互子粒子數”中的碰撞數量（>0）  
  ④ 最後在爆炸粒子中設置對應的“全局區分ID”（包含1即可），它就能在雨滴落地時感受到召喚了！
> 分配ID時一定要注意，自己召喚自己可能使粒子數量指數級增長，幾秒內卡死引擎！！
- 新增`全局粒子生成區域`，可以使用全局區分ID在特定地點召喚粒子。默認是方形區域，更改粒子系統後，還有球形、圖像、曲線、靜態網格體、骨骼網格體這些區域
- 新增`粒子交互場`，當前負數表示殺死粒子，正數表示在粒子進入該場的時候觸發子粒子交互事件，≥256會在場中持續觸發交互事件
- 新增`禮花粒子`，這是落葉粒子的改版，兩者都用上了MC粒子模板
- 更新了MC材質總類，一些粒子系統能對方塊染色了
- 新增`動態地形粒子`，由動態設計克隆器和遠景地形生成器改造而來
  - 每一幀都能更新地形，可以使用視頻紋理作爲地形貼圖，或者對生成參數打關鍵幀
  - 兼容動態設計效應器，並提供了“創建鏈接效應器”按鈕
  - 比遠景地形生成器多一個“生物羣系貼圖”，目前是根據圖片對方塊染色，之後會更新更多功能
- 修復了導播臺的DLSS選項修改後無法保存的bug
- 修復了統一粒子羣體在攝像機遠離又靠近時重新生成的bug
- 修復了全局力場激活休眠對象時導致靜態碰撞失效的bug
...
- FFmpeg已更新至最新版master build，添加了prores_ks_vulkan編碼器。不過，所有vulkan編碼在測試時都有閃爍/花屏問題，暫時無法修復
- 新增12bit 4:2:0、12bit 4:2:2、12bit 4:4:4三種色度抽樣選項。CPU H.265編碼支持所有9種選項，其他GPU編碼可選的條目如下所示：

|   | 8bit 4:2:0 | 10bit 4:2:0 | 10bit 4:2:2 | 10bit 4:4:4 | 12bit 4:2:0 |
| :---: | :---: | :---: | :---: | :---: | :---: |
| Nvidia | ✅ <br> nv12 | ✅ <br> p010le | ℹ️僅50系 <br> p210le | ✅ <br> yuv444p10msble | - |
| Intel | ✅ <br> nv12 | ✅ <br> p010le | ✅ <br> y210le | - | ✅ <br> p012le |
| AMD | ✅ <br> nv12 | ✅ <br> p010le | - | - | - |
| Apple | ✅ <br> nv12 | ✅ <br> p010le | ✅ <br> p210le | - | - |
| Vulkan | ✅ <br> nv12 | ✅ <br> p010le | - | - | - |

- 新增`分層渲染`，能夠把場景中的對象分成多個層，渲染多張背景透明的圖片。目前，僅單個層支持ffmpeg視頻編碼，且光影顏色可能有所不同，未來考慮使用MRG影片渲染圖改進分層渲染。
- 使用ProRes時勾選HDR或3D渲染，會主動改用ffmpeg編碼，現在能導出透明的HDR視頻了！需要注意的是，只有Apple GPU和Vulkan支持ProRes硬件加速，選中Nvidia/Intel/AMD GPU會退回基於CPU的prores_ks編碼
- Apple GPU確實不支持av1編碼，退回基於CPU的libsvtav1編碼
- “渲染帶HDR增益圖的jpg”即將轉正，已經移至頂部菜單欄的“調試與開發”主板塊。修復了渲染不在第0幀時的報錯問題，改進了HDR色彩管理流程。目前，僅剩下UI界面編輯器等少數功能尚不支持HDR增益圖
- 移除了實驗性功能`擴展模板序列以支持雙精度浮點和向量屬性乘數`。此功能在製作動畫時從未使用，而且難以編譯，在linux上安裝會報錯，不如直接移除
- 添加了本地化條目，能夠翻譯變量類別了
- 修復了“遠景地形生成器”在mac上因爲中文變量名而失效的bug。雖然內部重命名爲英文，但是本地化系統能把它翻譯成原模原樣的中文
- 修復了反斜槓路徑名在mac和linux上引發的錯誤
- 修復了mac和linux不能使用tkinter打開文件的問題，轉而使用系統自帶的文件瀏覽器
- 修復了mac和linux上可執行文件缺少權限的問題。使用安裝包或重新初始化插件時，都會爲ffmpeg和ultrahdr_app注入rwx權限
- 修復了mac和linux缺少內置的H.264與ProRes編碼器的問題，改用ffmpeg編碼。可惜的是，mac還缺少nDisplay插件，目前沒有替代方案，因此不能進行3D立體渲染
> 虛幻引擎震撼首發：[Apple ProRes Movie ...... 只在Windows構建中可用！](https://dev.epicgames.com/documentation/zh-cn/unreal-engine/movie-render-graph-nodes-in-unreal-engine#outputfiletype%E8%8A%82%E7%82%B9)  
> 不是哥們 這什麼NTR行爲啊喂🤣
- 更多mac和linux的兼容性問題見[系統要求](#系統要求)部分



#### 0.5-RC1
- Mineprep 0.5進入最後的收尾階段，預計在兩週內發佈
- 改進了安裝包，現在會顯示安裝所需的空間，能夠安裝精簡版，以及跳過爲其他平臺編譯的文件。
- 新建了一個倉庫 [Mineprep Lite](https://github.com/piggestpig/Mineprep-Lite)，專門存放精簡版插件。目前移除了源碼、音效、whisper、運動匹配相關資源，體積縮小1個G以上，同時保留了95%的核心功能，以GPLv3協議開源
- 調整了多個程序，修復一些在Mac/Linux上可能出現的bug
- 爲Linux編譯了實驗性功能。由於wsl2缺少Vulkan，暫時不能啓動虛幻引擎進行測試
- 開始編寫 [Minecraft Wiki](https://github.com/piggestpig/Unreal-Mineprep/wiki)，製作新手入門教程
- 【實驗性功能】增強了本地化系統，可以翻譯粒子參數了！至此，我們終於有能力翻譯所有的常見信息
- 【實驗性功能】新增快捷鍵，按insert嘗試翻譯鼠標下的文本。如果在[變量顯示名_VariableDisplayNames.csv](./Mineprep/插件贴图/变量显示名_VariableDisplayNames.csv)找到匹配條目，就替換文本
- 【實驗性功能】添加自定義快捷鍵時，能檢測按鍵衝突，並在右下角彈窗提示

#### 26w08a
- 新的動畫作品《撿到一隻快樂小幽靈2》現已發佈！第三集正在製作中，小幽靈的故事就要完結啦
  - 在製作動畫的過程中，我們檢驗了現有功能並修復了許多bug。新功能直接在工程文件中開發，注重參數化和拓展性
  - DLSS光線重構首次投入使用，極大提升了水面反射和半透明陰影的質量
  - 這是第一個使用正確的HDR色彩管理流程渲染的視頻，配合LabPBR材質，畫面相較於第一集有了質的飛躍
  - UE5.7本身就是一個很好的版本。一方面，材質參數面板十分穩定，大幅減少了閃退次數；另一方面，緩動曲線預設從動態設計序列下放到普通關卡序列，用起來非常方便
- 新增`鸚鵡`，`駱駝`，`駱駝屍殼`，`旱骨(枯骸)`及其NPC生物。鸚鵡NPC能根據隨機種子刷新皮膚，駱駝會自動扇耳朵
- 修改和添加了幾個攝像機晃動預設
- 修改和添加了一些粒子系統，還沒有放入生成器子面板中
- 添加了一些生物的動畫片段
- 添加了一些方塊模型
- 修復了飛行NPC在原地不動的bug。之後會進一步修改狀態樹
- 修復了插件面板在初始化時未能注入本地化材質參數名的bug
- 拓展了本地化系統，原先零零散散分佈在藍圖中的“簡單中英文選擇器”，也能被收錄在[變量顯示名_VariableDisplayNames.csv](./Mineprep/插件贴图/变量显示名_VariableDisplayNames.csv)裏面了。多行文本有一些問題，因爲換行符可能是\r\n而不是\n

#### 26w07a
- 要過年啦！！在這辭舊迎新之際，我們修復了近期最重大的bug，調整了一部分動畫相關功能
- 引擎自帶的Anim Mixer插件已被禁用。它會強制使用自定義動畫模式，使動畫藍圖失效。更多信息參見 [https://forums.unrealengine.com/t/ue5-7-preview-sequencer-forces-custom-mode-overrides-animation-blueprint/2663672](https://forums.unrealengine.com/t/ue5-7-preview-sequencer-forces-custom-mode-overrides-animation-blueprint/2663672)
- 沒有Anim Mixer插件，暫時無法在時間線上使用動畫序列控制新版觀衆人羣（UAF動畫框架）。“切換動作”事件仍然有效
- 爲媒體摳像播放器製作了動畫藍圖，用於混合默認動畫和時間線上的動畫序列
- 基礎生物模型和NPC生物新增`面向玩家/目標`、`面向角度/速度/始末距離`參數，可以自動轉頭面向最近的目標。部分舊的生物模型由於骨架不同，旋轉方向可能不對
- 騎乘者現在會自動對齊旋轉
- 村民NPC能根據隨機種子自動刷新皮膚了
- 遠景地形生成器新增`地形邊界衰減`參數，用於降低邊界高度
- 導播臺新增`優化/還原`按鈕，點擊可以優化簡單MC場景的性能
> Mineprep 0.5已經不知不覺做了快一年了，下週一定要發佈新的動畫，然後速速結束這個版本呀！！或許我選擇“荒野更新”這個大版本主題就是註定要拖更的 (´。＿。｀)  
> 荒野更新（x）  洞穴與山崖part3（√）

#### 26w06a
- 本週大幅改進了遠景地形生成器，添加了水面，公開了許多參數，能夠使用自定義`地形紋理`來控制地形的高度、植被密度和水域了
  - 平原、山脈和水面的高度會根據邊界框自動縮放，通過`平原/山脈/水面高度偏移`這個參數調整相對值，勾選`低窪處填充水面`或者使用地形紋理來生成水
  - 地形紋理的RGB通道分別對應高度/植被密度/水域，通常在0~1之間，也支持小於0或大於1的值  
  · R>0升高地面，R<0降低地面  
  · G>0增加植被，G<0減少植被  
  · B=1時直接轉換爲水域，0<B<1時對地形高度進行插值，如果最後的地表高度低於河面，纔會生成水
  - 提供了4組參數用於調整地形紋理：`高度/植被/河流偏移` `高度/植被/河流縮放` `高度/植被/河流乘方` `高度/植被/河流模糊`，每一個都是4D向量，分別對應RGBA通道
  - 默認在GPU上生成地形，速度非常快，便於調整參數。但是GPU地形經常會從編輯器中消失，渲染開始時需要花幾幀時間重新生成地形。取消勾選“在GPU上生成”，則會傳回穩定的CPU模型，不過生成速度很慢。
> 如果用普通的圖片作爲地形紋理，說不定有意想不到的驚喜！試試虛幻引擎自帶的各種貼圖吧，比如 3DMotionGraphicsThumbnail 這張圖片生成的效果還挺好看的

> 我目前沒找到什麼比較好的畫圖工具，虛幻引擎雖然有一個網格體繪製模式，裏面能畫紋理貼圖，但它真的好難用 ╯︿╰
- 新增`苦力怕`及其NPC生物
- 改進了LabPBR材質，現在會從高光貼圖中讀取次表面散射，植物搖擺不會自動添加次表面散射了
- 修復了一些動畫序列造成頭部異常旋轉的bug
- 將MC攝像機的根組件替換爲骨骼網格體，也就是說可以像生物一樣製作骨骼動畫了
- UE 5.7 的關卡序列似乎有bug，對內部組件打關鍵幀會因中文名而閃退，目前把生物的頭部重命名爲Head
- 媒體摳像播放器已更新Substrate材質
- 清理了一些舊資產
- 【實驗性功能】材質參數面板新增📄按鈕，點擊複製一份新的材質
- 【實驗性功能】按Ctrl+Alt+9 交換選中項的位置


#### 26w05a
- 本週我們將MCprep的Mesh Swap功能帶入虛幻引擎，這是盼望已久的最後一個核心功能！
- `更換模型（實例化）`
  - 位於導入世界的按鈕下面，目前有`分割不相鄰方塊的距離`、`分割爲獨立的Actors`、`自發光光源`、`生成輔助燈光`四個參數
  - 點擊後把大模型（燈籠、火把等離散的方塊）拆成小塊，每一塊都能單獨移動或刪除，同時支持還原爲大模型
  - 拆分後顯著提升了自發光照明效果，還有一鍵生成輔助燈光的功能
  - 實際上，拆出來的是動態網格體，不是實例化網格體。暫時不支持GPU距離場碰撞箱，也不能對它使用準備場景
- 導播臺新增`自定義控制檯變量`，公開了許多調整畫質的參數。比如，最新的r.MegaLights.ScreenTraces.MaxDistance 15減少了屏幕空間追蹤距離，緩解陰影閃爍問題
- MC天空現在會把時間保存到本地文件裏，重啓後自動讀取，比之前更可靠
- 優化了方塊體積雲，增大了視野範圍，遠處有淡出效果
- 4級動態材質和插件面板新增`覆層貼圖`，`覆層顏色`，`Stencil覆層貼圖遮罩`參數
- 拓展了材質系統，正在製作新的生物皮膚材質，用於多層貼圖和更好的2D表情動畫
- 修復了準備場景有時因空紋理而閃退的bug

#### 26w04a
- 本週材質大更新基本完成~ 清理了舊程序和舊參數
- 插件面板新增兩行自定義材質參數，可以手動輸入參數名
- 應用材質參數時有三個目標類型：`方塊（靜態網格體）`、`生物（骨骼網格體）`、`覆層材質`，默認勾選前兩個。覆層材質是新添加的類型，暫時不支持關鍵幀
- 準備場景時，會自動判斷黑白貼圖再應用顏色，始終對單個小方塊使用“簡單與複雜”碰撞
- 修復了低分辨率貼圖在一定距離處變模糊的問題
- 安裝插件時，會把引擎時間碼修改爲60fps。這將用於鏡頭試拍錄製器和新的防掉幀保護功能
- 方塊、基礎生物模型和NPC生物新增`騎乘者`參數，可以快速讓一個對象騎在另一個對象上
  - 每次添加新的乘客，會在`騎乘骨骼位`中尋找空位，然後把乘客附加到該骨骼上，使用指定的動畫
  - 勾選`分離(取消騎乘)`會解綁該乘客。再次點擊取消勾選會重新綁定，相當於刷新乘客狀態
  - 附加乘客時，會自動禁用兩者之間的碰撞，防止穿模後抽搐
  - 癱瘓會向上傳遞，也就是坐騎和乘客全部摔倒。斷肢不會傳遞，最好不要讓騎的骨骼斷開
  - 目前，不支持讓NPC騎在別的對象上
- 爲方塊添加了“癱瘓”事件，相當於開始物理模擬
- 點擊兩次基礎生物模型的“物理Ⅰ”、“物理Ⅱ”、“物理Ⅲ”、“物理Ⅴ”按鈕，會切換到癱瘓狀態
- 新增`拴繩`，包含繩子、拴繩結和物理約束
  - 繩子是視覺效果，會和場景碰撞，但不會影響場景中的其他對象
  - 物理約束則是實際的鏈接，可以懸掛或牽引正在物理模擬中的對象。它會以牽引端爲圓心進行約束（相當於懸掛），如果要牽引，建議把牽引端先放在對象身上，運行時再拉出來。
  - 可以把繩子和物理約束綁定到不同的骨骼上
  - 目前製作了`彈簧`、`硬約束`、`軟約束`三個快捷按鈕
- 附加組件新增`拴繩`和`騎乘上一個選中項`功能
  - 選中多個對象時，使用“拴繩”會把其他對象栓到最後一個選中項上
  - 選中多個對象時，使用“騎乘上一個選中項”會疊高高，從後往前依次騎乘上一項
- 【實驗性功能】右鍵細節面板中的材質參數，可以複製黏貼參數值和變量名了
- 【實驗性功能】增強了本地化系統，能夠修改引擎內置的文本了！在[變量顯示名_VariableDisplayNames.csv](./Mineprep/插件贴图/变量显示名_VariableDisplayNames.csv)的第一列添加了Namespace，用於區分不同板塊。勾選插件面板中的“朗讀鼠標下文本”後，會在日誌中打印讀取到的本地化信息，然後把Namespace、Key和Source String複製到csv文件的前三列，就能自定義翻譯文本了
- 【實驗性功能】右鍵點擊MC場景編輯器子面板的圖標，可以直接進入此編輯器模式了
- 更新了MC場景編輯器的操作方式，更符合原版MC的樣子了：
  - 移動鼠標將自動旋轉視角。按住左Alt鍵停止自動旋轉
  - 按TAB鍵切換爲不自動旋轉，此時需要按住鼠標中鍵才能旋轉視角（類似於Blender）
  - 由於虛幻引擎的限制，十字準星也會隨鼠標移動。按右Shift鍵在屏幕中間生成另一個十字準星
  - 按下鼠標左鍵/右鍵，能立即破壞/放置方塊，而不是在鬆開鼠標時才生效
  - 長按或拖動鼠標左鍵/右鍵，可以連續破壞/放置方塊
  - 修復了點擊物品欄時並未切換對應方塊的bug

#### 26w03a
- 重新設計了插件面板的材質參數，添加了大量新功能和參數行，每一行採用`🔲參數名【參數值】🔒🔗🔷`的格式，最頂上有`應用參數按鈕📥`和`按等級過濾參數🔍`
  - **🔲**：勾選框，是否用於準備場景、更換材質和`應用參數按鈕📥`。按住鼠標左鍵能批量刷選
  - **參數名**：點擊後應用此參數至選中項。雙擊時應用參數並保存更改
  - **參數值**：有數值滑條和顏色這兩種類型，右邊有返回默認值的小箭頭
  - **🔒**：點擊後解鎖數值滑條的滑動範圍限制。另外，無論有沒有解鎖，都可以用鍵盤輸入任意值。按住鼠標左鍵能批量刷選
  - **🔗**：即時同步，啓用後修改參數值能即時應用於選中項。按住鼠標左鍵能批量刷選
  - **🔷**：關鍵幀按鈕，左鍵對所有選中項添加關鍵幀，若參數名不存在則跳過。右鍵/中鍵會先應用參數值，再添加關鍵幀。
  - 右鍵“準備場景”等大按鈕能過濾相關參數。摺疊“高級設置”面板再展開會顯示全部參數
  - “附魔”、“植物搖擺”、“半透明”、“自發光”始終會應用相關參數
  - 更多功能正在開發中，暫不考慮靜態準備場景，未來會移除舊參數和相關程序
- 插件面板新增`半透明`按鈕，點擊後能把模型的材質轉換爲真正的半透明材質，適合染色玻璃、淡出消失等效果
  - 半透明材質基於原有的動態材質，也分1~4級，新增了`模糊`和`折射率`參數
  > Substrate材質讓非光追半透明也有了模糊效果，但是它受粗糙度控制，直接使用會造成大部分方塊變糊。因此，我們使用獨立的“模糊”參數來控制模糊程度；粗糙度則是影響高光
  - 1級：接受漫反射光照，粗糙度直接減少高光度。不透明度被連接至Substrate覆蓋權重，因此越透明的材質模糊上限越低。其他功能和普通的1級動態材質相同
  - 2級：在1級的基礎上，接受鏡面高光光照。沒有“次表面顏色RGB+透射A”參數。其他功能和普通的2級動態材質相同
  - 3級：粗糙度被連接至Substrate第二粗糙度，用於模糊高光效果。其他功能和普通的3級動態材質相同
  - 4級：這一等級不再使用Substrate覆蓋權重，而是添加了“次表面顏色RGB+透射A”參數，由SSS MFP（次表面散射平均自由程）控制透明度。不透明度則用於縮放SSS MFP，值爲0不一定能讓物體隱形。實際上，SSS MFP纔是Substrate半透明的正確用法，能夠充分體現模糊效果；假如未來Lumen能投射彩色半透明陰影，SSS MFP還會被用來控制陰影顏色。
  - 一般來說，方塊的折射效果很差，不需要動折射率。開了光線追蹤半透明後，效果會好一點，此時折射率>1纔會有模糊效果。
  - 普通的方塊或生物不需要半透明材質。用的多了會出現半透明排序問題，也就是各種奇怪的穿模。光線追蹤半透明是最簡單的解決方法，但是整個光影效果就變了，我通常很少使用
  - 目前，半透明材質都是單面的，不適合植物（雙面的排序問題實在太多了⊙﹏⊙）
  - 對半透明材質使用“準備場景”就能變回普通材質了
- 4級動態材質新增`間接光照貼圖`和`陰影貼圖`，前者可以修改反射/折射/自發光照明的顯示內容，後者可以修改陰影蒙板。比如說，讓生物在鏡子裏顯示另一種皮膚。默認不重載，也就是使用紋理貼圖。
- 新增`淡出消失`模板序列，適合半透明材質
- 修復了DLSS選項失效的bug。現在默認啓用光線重構，能大幅提升半透明陰影清晰度。修復了半透明陰影在遠處消失的問題
- 修復了畫質預設因爲本地化翻譯而損壞的bug。同時修改了畫質預設選項，移除了4*和“反射爲極高”，改爲
  - 3- 極高（快速Lumen）
  - 3  極高
  - 3+ 極高（特調畫質）
  - 4- 影視級
  - 4  影視級（特調畫質）
  - 4+ 影視級（增強lumen）
> 有了DLSS光線重構後，反射更加清晰，我們決定始終使用高質量半透明反射。默認渲染畫質變回 4 影視級（特調畫質）。如果需要節約顯存，3+ 極高（特調畫質）也是不錯的選擇


#### 26w02a
- 所有實驗性功能已升級至UE5.7！
  - 虛幻引擎製作了新的新手入門插件，不再需要舊版GuidedTutorials，已經將其從安裝包中移除
  - 添加了新的實驗性功能`擴展模板序列以支持雙精度浮點和向量屬性乘數`。這個早就做好了，在UE5.7中啓用
  - 添加了新的實驗性功能`爲材質參數面板添加關鍵幀按鈕和本地化翻譯`。引擎內置的InlineMaterialInstance插件經過大改，材質參數面板不再閃退了，但是無法通過之前的方法添加關鍵幀按鈕，於是我直接修改插件，並把它和本地化系統集成在一起
- 材質系統正在升級至Substrate！
  - 大部分材質的默認效果和以前保持一致，我們仍然沿用之前的“金屬度”、“高光度”等參數，便於調整
  - Substrate帶來了新的特性：現已支持LabPBR高光貼圖的8種特殊金屬F0；將高光度提高至>1可以進一步讓材質變成鏡子；次表面散射的透射A能更好地表示透光程度了
  - 然而，Substrate大幅增加了次表面散射的指令數，造成性能下降。於是，我們調整了1級動態材質，移除次表面散射，添加法線貼圖，適用於大部分方塊。現在，導入世界將使用1級動態材質來準備場景，生物也依舊使用1級動態材質。
- 觀衆人羣已升級至UAF動畫框架，大幅提高了在編輯器中的性能！
  - 使用方法和以前一樣，繼承了大部分功能，在必要時退回舊版動畫藍圖
  - UAF目前不支持物理交互。使用衝擊和斷肢會自動癱瘓，然後退回動畫藍圖
  - 切換動作暫不支持播放速率和初始位置，支持播放次數和自定義鍵控動畫組
  - 額外旋轉目前支持z軸的0~180°，和以前一樣，設置爲90表示面向x軸
  - 暫不支持在時間線上使用控制綁定，FK綁定也不行
- 改進了媒體摳像散佈器，和觀衆人羣散佈器一樣，可以重載動畫，自適應面向目標
- UE 5.7 不能對許多數組變量打關鍵幀了。修復了受傷變紅模板失效的問題。添加了破壞方塊的模板序列
- “遠景地形生成器”能投射光線追蹤陰影了，而且性能提升了幾十倍，在達到1800萬方塊上限前都能瞬間生成，極其震撼！！同時優化了內部的點生成器，現在完全運行在GPU上
- 修復了“簡單跟隨路徑移動控制器”失效的bug
- 修復了無綁定渲染和DLSS衝突造成閃退的bug。另外，經過測試，UE5.7的DLSS光線重構沒有渲染問題了
...
- “注入本地化變量名”轉正了！相關按鈕已經從頂部菜單欄移除，不再是實驗性功能，而是在切換語言時自動運行。我們第一次使用了UE內置的polyglot本地化系統，且第一次翻譯了枚舉值。這能在25ms內注入1000條變量名，速度極快，無需編譯。變量名保存在[變量顯示名_VariableDisplayNames.csv](./Mineprep/插件贴图/变量显示名_VariableDisplayNames.csv)中
...
- 新增`豬人(豬布林)`，`殘暴豬人(豬布林蠻兵)`，`衛道士`，`掠奪者`及相關動作庫。
  - 在放置NPC時，豬人(豬布林)默認手持金劍，殘暴豬人(豬布林蠻兵)手持金斧，衛道士手持鐵斧，掠奪者手持弩。
